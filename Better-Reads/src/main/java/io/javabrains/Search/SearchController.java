...
public class SearchController {
	...
	public String getSearchResults(@RequestParam String query, Model model) {
		Mono<SearchResults> resultsMono = this.webClient.get().uri("?q={query}", query).retrieve()
				.bodyToMono(SearchResults.class);
		SearchResults results = resultsMono.block();
		List<SearchResultBook> books = results.getDocs().stream().limit(10).map(bookResult -> {
